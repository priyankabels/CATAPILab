...
async function getBreedInfo()
{
    ...
    let breedInfo=[];
    const response =await fetch(`https://api.thecatapi.com/v1/images/search?limit=10&breed_ids=${breedSelect.value}&api_key=${API_KEY}`);
    ...
    breedInfo.forEach((breedObj)=>{
          Carousel.appendCarousel(Carousel.createCarouselItem(breedObj.url,"Cat Image",breedObj.id));
       
    });
    console.log(breedInfo);
    // Check if breedInfo data exists
